...
export default function LiveInventoryPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Unable to load inventory from the database.")
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      <Card><SectionHeader title="Inventory Value by Category" subtitle="Calculated from live batches and available barcodes" /><ResponsiveContainer width="100%" height={180}><BarChart data={categoryData}><CartesianGrid strokeDasharray="4 4" /><XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip content={<ChartTooltip />} /><Bar dataKey="sales" name="Inventory value" fill="#1e8a4a" radius={[5, 5, 0, 0]} /></BarChart></ResponsiveContainer></Card>
      <Card><SectionHeader title="Units Received by Supplier" subtitle="Live stock-batch records" /><ResponsiveContainer width="100%" height={180}><BarChart data={dataset.supplierUnits}><CartesianGrid strokeDasharray="4 4" /><XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip content={<ChartTooltip />} /><Bar dataKey="units" name="Units received" fill="#34d399" radius={[5, 5, 0, 0]} /></BarChart></ResponsiveContainer></Card>
    </div>
    <Card>
