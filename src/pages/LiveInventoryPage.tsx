...
export default function LiveInventoryPage({ initialStatus }: { initialStatus?: "attention" | InventoryRow["stock_status"] } = {}) {
  ...
  return <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
      <Card><SectionHeader title={t("inventoryPage.chartValueByCategory")} subtitle={t("inventoryPage.chartValueByCategorySubtitle")} /><ResponsiveContainer width="100%" height={200}><BarChart data={categoryData} margin={{ bottom: 28 }}><CartesianGrid strokeDasharray="4 4" stroke="var(--border)" /><XAxis dataKey="name" tick={{ fontSize: 10, fill: "var(--ink-muted)" }} interval={0} angle={-25} textAnchor="end" height={50} /><YAxis tick={{ fontSize: 10, fill: "var(--ink-muted)" }} /><Tooltip content={<ChartTooltip />} /><Bar dataKey="sales" name={t("inventoryPage.chartInventoryValue")} fill="#1e5fa8" radius={[5, 5, 0, 0]} /></BarChart></ResponsiveContainer></Card>
      <Card><SectionHeader title={t("inventoryPage.chartUnitsBySupplier")} subtitle={t("inventoryPage.chartUnitsBySupplierSubtitle")} /><ResponsiveContainer width="100%" height={200}><BarChart data={dataset.supplierUnits} margin={{ bottom: 28 }}><CartesianGrid strokeDasharray="4 4" stroke="var(--border)" /><XAxis dataKey="name" tick={{ fontSize: 10, fill: "var(--ink-muted)" }} interval={0} angle={-25} textAnchor="end" height={50} /><YAxis tick={{ fontSize: 10, fill: "var(--ink-muted)" }} /><Tooltip content={<ChartTooltip />} /><Bar dataKey="units" name={t("inventoryPage.chartUnitsReceived")} fill="#60a5fa" radius={[5, 5, 0, 0]} /></BarChart></ResponsiveContainer></Card>
    </div>
    <div ref={tableRef}>
    <Card>
