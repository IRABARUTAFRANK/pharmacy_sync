...
export default function OverviewPage({
  ...
  // Absent for the org-wide combined dashboard embedded in OrganizationPage
  // (there's no single "this branch's onboarding" to show there) -- present
  ...
  const [checklistDismissed, setChecklistDismissed] = useState(() => !userId || isOnboardingChecklistDismissed(userId))

  useEffect(() => {
    if (checklistDismissed || !userId) return
  ...
  return (
    <>
      {onboarding && !checklistDismissed && userId && onNavigate && (
        <OnboardingChecklist
          progress={onboarding}
            ...
            <Card style={heroCardStyle}>
              ...
              <ResponsiveContainer width="100%" height={185}>
                <BarChart data={data.dailyTransactions} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                  ...
                  {/* Recharts' tooltip `label` is the raw axis VALUE (the
                      untranslated "overviewPage.dayXxx" key), not the tick's
                      own display text -- tickFormatter above only affects
                      what's drawn on the axis itself. Without labelFormatter
                      here too, the tooltip showed the literal key string
                      instead of "Sat". */}
                  <Tooltip content={<ChartTooltip />} labelFormatter={(key: unknown) => t(key as TranslationKey)} />
                  <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11, color: "var(--ink-mid)" }} />
