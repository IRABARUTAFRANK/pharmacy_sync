...
export async function loadVatByMonth(months = 8): Promise<MonthlyVatPoint[]> {
  const { data, error } = await supabase.rpc("analytics_vat_by_month", { p_months: months })
...
export async function listComplianceTransactions(from: string, to: string, limit = 200): Promise<ComplianceTransaction[]> {
  const { data, error } = await supabase.rpc("list_compliance_transactions", { p_from: from, p_to: to, p_limit: limit })
